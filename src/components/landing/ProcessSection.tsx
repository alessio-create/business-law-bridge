...
const ProcessSection = () => (
  <section id="processo" className="px-6 py-20 md:px-20 bg-card">
    <div className="max-w-[1200px] mx-auto text-center">
      <span className="text-primary font-bold text-xs uppercase tracking-widest mb-3 block">Come Lavoriamo</span>
      <h2 className="text-foreground text-3xl md:text-4xl font-black mb-16">Il nostro processo in 3 step</h2>
      <div className="relative grid grid-cols-1 md:grid-cols-3 gap-12">
        <div className="hidden md:block absolute top-8 left-[16%] right-[16%] h-0.5 bg-border z-0" />
        {steps.map((s) => (
          <div key={s.num} className="relative z-10 flex flex-col items-center gap-6">
            <div className="w-16 h-16 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-2xl font-bold shadow-xl border-4 border-card">
              {s.num}
            </div>
